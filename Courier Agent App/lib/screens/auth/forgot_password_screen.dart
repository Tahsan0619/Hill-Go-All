...
class _ForgotPasswordScreenState extends State<ForgotPasswordScreen> {
  ...
  Future<void> _next() async {
    ...
    if (_step == 1) {
      ...
      final ok = await auth.sendPasswordResetOtp(_contact.text.trim());
      if (!mounted) return;
      if (ok) {
        setState(() => _step = 2);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(auth.error ?? 'Could not send the reset code')),
        );
      }
      return;
    }
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        _step == 1
                            ? 'Enter the phone number linked to your courier account.'
                            : _step == 2
                      ...
                      if (_step == 1) ...[
                        ...
                        Center(
                          child: ResendTimer(
                            onResend: () => context
                                .read<AuthProvider>()
                                .sendPasswordResetOtp(_contact.text.trim()),
                          ),
                        ),
                      ] else ...[
