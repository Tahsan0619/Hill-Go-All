  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: const HillGoAppBar(title: 'Vehicle Info', showBack: true, showBell: false), body: Form(
      key: _form, child: ListView(padding: const EdgeInsets.all(AppSpacing.screenPadding), children: [
        ...
        const SizedBox(height: 16),
        DropdownButtonFormField<String>(initialValue: _type, decoration: const InputDecoration(labelText: 'Vehicle type'), items: const ['Motorbike', 'Bicycle', 'Car', 'Van'].map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(), onChanged: (v) => setState(() => _type = v!)),
        const SizedBox(height: 28),
        PrimaryButton(label: 'Save vehicle details', loading: provider.loading, onPressed: () async {
          if (!_form.currentState!.validate()) return;
          final messenger = ScaffoldMessenger.of(context);
          final nav = Navigator.of(context);
          await provider.updateVehicle(name: _name.text.trim(), plate: _plate.text.trim(), type: _type);
          if (!mounted) return;
