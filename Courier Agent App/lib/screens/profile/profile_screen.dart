    ...
    body: Consumer<ProfileProvider>(builder: (context, provider, _) {
      ...
      return ListView(padding: const EdgeInsets.all(AppSpacing.screenPadding), children: [
        Center(child: Column(children: [
          ...
          Row(mainAxisSize: MainAxisSize.min, children: [Text(user.name, style: AppTextStyles.h2), const SizedBox(width: 6), const Icon(Icons.verified, color: AppColors.verified, size: 20)]),
        ...
        Row(children: [
          ...
          const SizedBox(width: 12), Expanded(child: _stat('Active time', provider.activeTime, Icons.timer_outlined)),
        ]),
        const SizedBox(height: AppSpacing.xl),
        const SectionLabel('Fleet & Security'), const SizedBox(height: 8),
        _nav('Vehicle Info', '${user.vehicleName} · ${user.vehiclePlate}', Icons.two_wheeler_outlined, '/profile/vehicle'),
        _nav('Document Status', 'All documents verified', Icons.verified_user_outlined, '/profile/documents'),
        const SizedBox(height: AppSpacing.lg), const SectionLabel('Preferences'), const SizedBox(height: 8),
        _nav('Notification Preferences', 'Push and email settings', Icons.notifications_outlined, '/profile/notifications'),
        _nav('App Language', provider.language, Icons.language_outlined, '/profile/language'),
        ...
        const SizedBox(height: AppSpacing.lg),
        Center(child: Text('App Version 4.2.1-stable', style: AppTextStyles.caption)),
      ]);
    }),
  );
