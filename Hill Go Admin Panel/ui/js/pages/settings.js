  ...
  const render = () => {
    ...
    root.innerHTML = `
      <div class="mb-6">
        ...
        <p class="text-sm text-outline mt-1">Persisted in local browser storage with the rest of the admin store.</p>
      </div>
      <form id="settings-form" class="bg-white rounded-xl border shadow-sm p-6 max-w-xl space-y-4">
        ...
        <div class="flex justify-end gap-2 pt-2">
          <button type="button" id="reset-store" class="px-4 py-2 rounded-lg border border-red-200 text-error text-sm font-semibold">Reset all mock data</button>
    ...
    root.querySelector('#reset-store')?.addEventListener('click', async () => {
      const ok = await U.confirmDialog({
        ...
        message: 'This clears localStorage and restores the original seed (districts, payouts, KYC, pricing). Continue?',
        danger: true,
        confirmLabel: 'Reset',
      });
      if (!ok) return;
      S.resetData();
      U.notice('Mock data restored');
      Router.go('/overview');
    });
  };
  render();
