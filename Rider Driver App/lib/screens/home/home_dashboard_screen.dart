...
class _HomeDashboardScreenState extends State<HomeDashboardScreen> {
  ...
  void _watchOffers(DriverProvider driver) {
    _offerPoll?.cancel();
    if (!driver.isOnline || driver.activeTrip != null) return;
    _offerPoll = Timer.periodic(const Duration(seconds: 3), (_) async {
      if (!mounted) return;
      final d = context.read<DriverProvider>();
      if (!d.isOnline || d.activeTrip != null || d.incomingOffer != null) return;
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () {
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Safety center opened (mock)')),
                );
              },
            ),
          ],
