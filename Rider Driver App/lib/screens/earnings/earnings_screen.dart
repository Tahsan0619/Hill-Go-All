...
class _EarningsScreenState extends State<EarningsScreen> {
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 6),
                                  Text(
                                    'High demand expected in North District between 5 PM and 8 PM tonight.',
  ...
  void _showCashOut(BuildContext context, DriverProvider driver) {
    final amountCtrl = TextEditingController(
      text: (driver.earnings?.currentBalance ?? 0).toStringAsFixed(2),
    );
            ...
            builder: (context, setModal) {
              return Column(
                ...
                children: [
                  Text('Instant Pay', style: AppTextStyles.title),
                  const SizedBox(height: 8),
                  Text('Mock payment sheet — no real charges.', style: AppTextStyles.bodySecondary),
                  const SizedBox(height: 16),
                  AppTextField(
                    label: 'Amount',
                  ...
                  SwitchListTile(
                    ...
                    subtitle: const Text('Toggle off to simulate failure'),
                    value: driver.simulateCashOutSuccess,
                    onChanged: (v) {
                      driver.setSimulateCashOutSuccess(v);
                      setModal(() {});
                    },
                  ),
                  const SizedBox(height: 8),
                  AccentButton(
                    ...
                    onPressed: () async {
                      ...
                      final ok = await driver.cashOut(amount);
                      if (!context.mounted) return;
                      Navigator.pop(context);
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(ok ? 'Cash out successful' : (driver.error ?? 'Failed')),
                        ),
                      );
                    },
                  ),
